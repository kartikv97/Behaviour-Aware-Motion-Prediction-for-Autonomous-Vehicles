...
def getDirectFrameDict(frames_list):

    # for agent in frames_list:
    content = np.array(frames_list)
    now_dict = {}
    for row in content:
        # instance = {row[1]:row[2:]}
        n_dict = now_dict.get(row[0], {})
...
def generate_test_data(frames_list):
    # now_dict = get_frame_instance_dict(pra_file_path)
    now_dict = getDirectFrameDict(frames_list)
    frame_id_set = sorted(set(now_dict.keys()))
...
def generate_data(frames_list, pra_is_train=False):
    all_data = []
    all_adjacency = []
    all_mean_xy = []
    # for file_path in pra_file_path_list:
    if pra_is_train:
        now_data, now_adjacency, now_mean_xy = generate_train_data(frames_list)
    else:
        now_data, now_adjacency, now_mean_xy = generate_test_data(frames_list)
    ...
    return [all_data, all_adjacency, all_mean_xy]
