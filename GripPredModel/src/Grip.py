...
max_x = 1. 
max_y = 1. 
history_frames = 1 # 3 second * 2 frame/second
future_frames = 1 # 3 second * 2 frame/second
...
lr_decay_epoch = 5
dev = 'cuda:0'
work_dir = '/home/kartik/Documents/CMSC818B/FinalProject/Behaviour-Aware-Motion-Prediction-for-Autonomous-Vehicles/GripPredModel/models/'
...
def data_loader(frames_list, graph_args,pra_batch_size=128, pra_shuffle=False, pra_drop_last=False, train_val_test='train'):
	feeder = Feeder(data_list=frames_list, graph_args=graph_args, train_val_test=train_val_test)
			...
			for n_pred, n_mean_xy, n_data, n_mask in zip(now_pred, now_mean_xy, now_ori_data, now_mask):
				...
				for time_ind, n_pre in enumerate(n_pred[:, :num_object], start=1):
					...
					for info, pred in zip(n_dat, n_pre+n_mean_xy):
						...
						result = list(information)+list(pred)
						# print(result)
...
# 		)
from dataProcess import generate_data
import glob
...
def run_test(pra_model,frames_list,graph_args):
	# test_file_path_list = sorted(glob.glob(os.path.join(data_root, 'test/*.txt')))
	# # loader_test = 	generate_data(test_file_path_list, pra_is_train=False)
	# # print(loader_test)
	loader_test = data_loader(frames_list,graph_args, pra_batch_size=batch_size_test, pra_shuffle=False, pra_drop_last=False, train_val_test='test')
	...
	Final_Result = test_model(pra_model, list(loader_test))
	return Final_Result
...
if __name__ == '__main__':
	...
	t1 = time.time()
	# run_trainval(model, pra_traindata_path='D:/Grip/GRIP-master/train.pkl', pra_testdata_path='D:/Grip/GRIP-master/test.pkl')
